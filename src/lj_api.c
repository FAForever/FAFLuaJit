...
int FAlua_type(lua_State *L, cTValue *o) {
    int FATypes[] = {0,1,1,2,4,11,9,10,7,-1,-1,5,8,-1};
    if (tvisfunc(o))
        if (iscfunc(funcV(o))) return 6;
    if (tvisnumber(o)) return 3;
    auto tt = FATypes[~itype(o)];
    lj_assertL(tt >= 0, "bad tag conversion");
    return tt;
}

LUA_API int lua_typeF(lua_State *L, int idx)
{
    cTValue *o = index2adr(L, idx);
    if (o == niltv(L))
        return LUA_TNONE;
    return FAlua_type(L, o);
}

LUA_API int lua_traceback(lua_State *L, const char *msg, int level)
{
